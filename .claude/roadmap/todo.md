...
| [3 — Services](phase-3-services.md) | Web server, databases and caches with generated config | T30–T38, T99 | 15 / 17 | **M3** caddy + mariadb + redis healthy in under 10 s warm |
...
| **M3's tail** — the warm median is inside ten seconds on all three, and two Linux rounds of five were 11.8 s and 15.1 s | nothing, and it is now explained: MariaDB 11.4 generating a 4096-bit RSA key at every start for a TLS nobody configured, not cold I/O and not the sequential walker. **T99** gives the server a certificate of this home's instead; the row stays until a `bench (ubuntu-latest)` run on T99 says what the tail became | [phase 3](phase-3-services.md) |
