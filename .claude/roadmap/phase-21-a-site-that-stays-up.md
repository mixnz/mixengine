...
- [x] **T167a** The wire. `ServiceSummary.stopped_by` (`never` | `person` | `daemon`, optional per
      ADR 0019), `SaveResources` / `SaveResourcesSet`, and the methods `service.save_resources` and
      `service.set_save_resources`.
- [x] **T167b** Idle off by default. Every `Recipe::idle_default()` answers `None`; T70/T70a's numbers
...
- [x] **T167c** `mix service save-resources [--on|--off]`.
- [x] **T167d** `stopped_by` filled on every `ServiceSummary` for a stopped service.
- [x] **T167e** The front end comes back with the daemon. A front end is created with `autostart` on
...
- [x] **T167f** *This site is starting…* — the page a front end serves for a site with a backend
      whose upstream is down, instead of the empty-site welcome page.
- [x] **T167g** MixLab: *Save battery* in Settings, keep-warm gone from the project form and table,
