 ...
 * mark is none of that: it is a fixed shape owned by someone else, filled rather than drawn at a
 * house weight, and recognised only when it is drawn exactly. So they don't go through
 * {@link Icon}, and adding one means pasting the official outline rather than redrawing it.
 *
 * Each is the monochrome mark from Simple Icons (CC0). Monochrome is what makes them usable here:
 * the fill is `currentColor`, so the brand colour comes from the CSS beside them (`.kind-*`) and
...
const BRAND_MARKS: Record<DbKind, { viewBox: string; path: string; strokeWidth?: number }> = {
  /* The dolphin alone, lifted out of the logo Simple Icons ships — that one is the full lockup,
     ...
     `viewBox` that crops to them.

     It is also line art where the other three are solid: the dolphin is drawn as hairline contours
     about 0.15 units wide, which land under half a pixel at this size and vanish. Stroking the same
     colour over the fill thickens every contour enough to survive — the outline stays the real one,
     it is only inked heavier. The weight is chosen so the tight gaps around the head close into
     solid mass while the wider ones along the belly stay open, which is what still reads as a
     dolphin once it is twenty pixels tall. */
  mysql: {
    viewBox: "14.1 3.7 10 10",
    strokeWidth: 0.7,
    path: "M16.405 5.501c-.115 0-.193.014-.274.033v.013h.014c.054.104.146.18.214.273.054.107.1.214.154.32l.014-.015c.094-.066.14-.172.14-.333-.04-.047-.046-.094-.08-.14-.04-.067-.126-.1-.18-.153zM23.224 11.311c-.535-.014-.95.04-1.297.188-.1.04-.26.04-.274.167.055.053.063.14.11.214.08.134.218.313.346.407.14.11.28.216.427.31.26.16.555.255.81.416.145.094.293.213.44.313.073.05.12.14.214.172v-.02c-.046-.06-.06-.147-.105-.214-.067-.067-.134-.127-.2-.193a3.223 3.223 0 00-.695-.675c-.214-.146-.682-.35-.77-.595l-.013-.014c.146-.013.32-.066.46-.106.227-.06.435-.047.67-.106.106-.027.213-.06.32-.094v-.06c-.12-.12-.21-.283-.334-.395a8.867 8.867 0 00-1.104-.823c-.21-.134-.476-.22-.697-.334-.08-.04-.214-.06-.26-.127-.12-.146-.19-.34-.275-.514a17.69 17.69 0 01-.547-1.163c-.12-.262-.193-.523-.34-.763-.69-1.137-1.437-1.826-2.586-2.5-.247-.14-.543-.2-.856-.274-.167-.008-.334-.02-.5-.027-.11-.047-.216-.174-.31-.235-.38-.24-1.364-.76-1.644-.072-.18.434.267.862.422 1.082.115.153.26.328.34.5.047.116.06.235.107.356.106.294.207.622.347.897.073.14.153.287.247.413.054.073.146.107.167.227-.094.136-.1.334-.154.5-.24.757-.146 1.693.194 2.25.107.166.362.534.703.393.3-.12.234-.5.32-.835.02-.08.007-.133.048-.187v.015c.094.188.188.367.274.555.206.328.566.668.867.895.16.12.287.328.487.402v-.02h-.015c-.043-.058-.1-.086-.154-.133a3.445 3.445 0 01-.35-.4 8.76 8.76 0 01-.747-1.218c-.11-.21-.202-.436-.29-.643-.04-.08-.04-.2-.107-.24-.1.146-.247.273-.32.453-.127.288-.14.642-.188 1.01-.027.007-.014 0-.027.014-.214-.052-.287-.274-.367-.46-.2-.475-.233-1.238-.06-1.785.047-.14.247-.582.167-.716-.042-.127-.174-.2-.247-.303a2.478 2.478 0 01-.24-.427c-.16-.374-.24-.788-.414-1.162-.08-.173-.22-.354-.334-.513-.127-.18-.267-.307-.368-.52-.033-.073-.08-.194-.027-.274.014-.054.042-.075.094-.09.088-.072.335.022.422.062.247.1.455.194.662.334.094.066.195.193.315.226h.14c.214.047.455.014.655.073.355.114.675.28.962.46a5.953 5.953 0 012.085 2.286c.08.154.115.295.188.455.14.33.313.663.455.982.14.315.275.636.476.897.1.14.502.213.682.286.133.06.34.115.46.188.23.14.454.3.67.454.11.076.443.243.463.378z",
  },
  postgres: {
    viewBox: "0 0 24 24",
  ...
  /* NOT a brand mark, and the one exception to the rule above: this is a plain database cylinder
     drawn here, standing in for SQLite's own logo.
     The official outline has to be pasted in from Simple Icons the way the other four were — it is
     not in this tree and redrawing a brand from memory produces a shape that is recognisably not
     the logo, which is worse than an obvious placeholder. Until then this reads as "a database",
     which is at least true. */
  sqlite: {
    viewBox: "0 0 24 24",
    path: "M12 2C7.6 2 4 3.34 4 5s3.6 3 8 3 8-1.34 8-3-3.6-3-8-3zM20 8.5c0 1.66-3.6 3-8 3s-8-1.34-8-3V12c0 1.66 3.6 3 8 3s8-1.34 8-3V8.5zM20 15.5c0 1.66-3.6 3-8 3s-8-1.34-8-3V19c0 1.66 3.6 3 8 3s8-1.34 8-3v-3.5z",
  },
  redis: {
    viewBox: "0 0 24 24",
  ...
  /* NOT a brand mark, same exception SQLite is above: four bars of rising height, standing in for
     ClickHouse's own logo until its official Simple Icons outline is pasted in here. Guessing at
     that outline from memory would draw a shape that is recognisably not the logo, worse than an
     obvious placeholder — see the SQLite entry's own note. */
  clickhouse: {
    viewBox: "0 0 24 24",
...
export interface DatabaseIconProps extends Omit<SVGProps<SVGSVGElement>, "viewBox" | "children"> {
  /** Which engine's logo to draw. */
  kind: DbKind;
  /** Edge length as any CSS length, defaulting to `1em` so the logo tracks the font size of
   * whatever it sits in — the same rule the line icons follow. */
  size?: number | string;
}

/** The logo of one database engine — MySQL's dolphin, Postgres's elephant, Mongo's leaf, Redis's
 * stack — drawn in `currentColor` so the surrounding rule picks its brand colour.
 *
 * `aria-hidden` like every other icon here: it decorates a control or a row that already carries
...
export function DatabaseIcon({ kind, size = "1em", className, ...rest }: DatabaseIconProps) {
  ...
  return (
    <svg
      ...
      height={size}
      fill="currentColor"
      stroke={mark.strokeWidth ? "currentColor" : "none"}
