-- no-transaction
-- Go is the sixth runtime kind — roadmap task **T27d**, the design's D2.
--
-- `runtime_installs.kind` carries `CHECK (kind IN ('php', 'node', 'python', 'ruby', 'composer'))`
-- from 0019, and SQLite cannot alter a CHECK in place, so the table is rebuilt exactly as 0019
-- rebuilt it: copy out, copy back, drop, rename. `services.runtime_install_id` references this
...
-- gives.
--
-- Every column is 0019's: no migration between it and this one has touched this table. SQLite
...
CREATE TABLE runtime_installs_new (
    ...
    kind                   TEXT    NOT NULL CHECK (kind IN ('php', 'node', 'python', 'ruby', 'go', 'composer')),
