-- no-transaction
-- Java is the seventh runtime kind — roadmap task **T27e**, the design's D2.
--
-- `runtime_installs.kind` carries `CHECK (kind IN ('php', 'node', 'python', 'ruby', 'go',
-- 'composer'))` from 0025, and SQLite cannot alter a CHECK in place, so the table is rebuilt exactly
-- as 0025 rebuilt it: copy out, copy back, drop, rename. `services.runtime_install_id` references
-- this table with `ON DELETE RESTRICT`, which is why foreign keys are off for the drop and
...
-- gives.
--
-- Every column is 0025's: no migration between it and this one has touched this table. SQLite
-- drops a table's indexes with the table, so the partial unique index — one default per kind — is
-- recreated by name below. No trigger has ever been defined on this table.
...
CREATE TABLE runtime_installs_new (
    ...
    kind                   TEXT    NOT NULL CHECK (kind IN ('php', 'node', 'python', 'ruby', 'go', 'java', 'composer')),
