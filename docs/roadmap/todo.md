...
| [26 — A window that builds in under ten](phase-26-a-window-that-builds-in-under-ten.md) | The longest leg of `build` finishes in ten minutes, with no cache and no change to what a tag builds | T173a–T173c | 0 / 3 | **M26** `window (windows-latest)` finishes in 10 minutes or less and `build`'s artifacts and probes are unchanged |
